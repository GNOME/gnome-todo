...
function modify_manifest() {
    ...
    for file in `grep -rl "org\.gnome\.Todo" data/ po/ plugins/ src/`
    do
        echo "        Modifying $file"
    ...
    sed -i "s,\"url\" : \"https://gitlab.gnome.org/GNOME/$PROJECT_NAME\",\"branch\" : \"<<current>>\"\,,g" $FLATPAK_DEVEL_MANIFEST
    sed -i "s,\"url\" : \"https://gitlab.gnome.org/GNOME/\",\"path\" : \".\",g" $FLATPAK_DEVEL_MANIFEST
