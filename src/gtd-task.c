                            ...
                            gboolean  complete)
{
  ...
  /* Update completion date to date now if marked as complete */
  if (complete && priv->completion_date)
    priv->completion_date = g_date_time_new_now_local();

  /* Clear task completion date if it is unmarked complete */
  if (!complete && priv->completion_date)
